...
const JWT_SECRET = process.env.JWT_SECRET || 'clawdef-' + Math.random().toString(36).slice(2);
...
db.exec(`
  CREATE TABLE IF NOT EXISTS token_usage (
    ...
    session_id TEXT NOT NULL, provider TEXT, model TEXT,
    input_tokens INTEGER DEFAULT 0, output_tokens INTEGER DEFAULT 0,
    cache_read_tokens INTEGER DEFAULT 0, cache_write_tokens INTEGER DEFAULT 0,
    total_tokens INTEGER DEFAULT 0,
    cost_input REAL DEFAULT 0, cost_output REAL DEFAULT 0,
    cost_cache_read REAL DEFAULT 0, cost_cache_write REAL DEFAULT 0, cost_total REAL DEFAULT 0,
    tool_calls TEXT, message_id TEXT
  );
  CREATE TABLE IF NOT EXISTS tool_calls (
    id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT NOT NULL,
    session_id TEXT NOT NULL, message_id TEXT, tool_name TEXT NOT NULL,
    tool_args TEXT, skill_name TEXT,
    tokens_in_request INTEGER DEFAULT 0, tokens_in_response INTEGER DEFAULT 0
  );
  CREATE TABLE IF NOT EXISTS skill_events (
    id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT NOT NULL,
    skill_name TEXT NOT NULL, event_type TEXT NOT NULL, details TEXT
  );
  CREATE TABLE IF NOT EXISTS alerts (
    id INTEGER PRIMARY KEY AUTOINCREMENT, timestamp TEXT NOT NULL,
    level TEXT NOT NULL, category TEXT NOT NULL, title TEXT NOT NULL,
    message TEXT, resolved INTEGER DEFAULT 0, resolved_at TEXT
  );
  CREATE TABLE IF NOT EXISTS budgets (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL UNIQUE, period TEXT NOT NULL DEFAULT 'daily',
    token_limit INTEGER DEFAULT 0, cost_limit REAL DEFAULT 0,
  ...
  CREATE TABLE IF NOT EXISTS model_health (
    ...
    last_error TEXT, last_check TEXT, consecutive_failures INTEGER DEFAULT 0,
    total_requests INTEGER DEFAULT 0, total_errors INTEGER DEFAULT 0,
    avg_latency_ms REAL DEFAULT 0
  );
  CREATE TABLE IF NOT EXISTS model_failover_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    timestamp TEXT NOT NULL, from_model TEXT, to_model TEXT,
    reason TEXT, auto BOOLEAN DEFAULT 1
  );
  CREATE TABLE IF NOT EXISTS auto_optimize_log (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    timestamp TEXT NOT NULL, action TEXT NOT NULL,
    from_model TEXT, to_model TEXT, reason TEXT,
    tokens_before INTEGER DEFAULT 0, tokens_after INTEGER DEFAULT 0,
    budget_remaining_pct REAL DEFAULT 0
  );
...
if (db.prepare('SELECT COUNT(*) as c FROM admin_users').get().c === 0) {
  db.prepare('INSERT INTO admin_users (username,password_hash,role,created_at) VALUES (?,?,?,?)')
    .run('admin', bcrypt.hashSync('admin', 10), 'admin', new Date().toISOString());
}
...
function getCurrentModel() {
  return (readJson(OPENCLAW_CONFIG)||{}).agents?.defaults?.model?.primary || '';
}
...
// ─── Failover Engine ───
function getFailoverConfig() { return (readJson(OPENCLAW_CONFIG)||{}).clawdef?.failover || { enabled:false, priority:[], check_interval_ms:60000, max_failures:3 }; }
function setFailoverConfig(config) { const cfg=readJson(OPENCLAW_CONFIG)||{}; if(!cfg.clawdef) cfg.clawdef={}; cfg.clawdef.failover=config; writeJson(OPENCLAW_CONFIG,cfg); }
...
// ─── Config ───
function getOpenClawConfig() { return readJson(OPENCLAW_CONFIG) || {}; }
function saveOpenClawConfig(cfg) { writeJson(OPENCLAW_CONFIG, cfg); }
...
function emergencyShutdown() {
  try { execSync('openclaw gateway stop',{timeout:10000}); return {success:true,message:'Gateway stopped'}; }
  catch { try{execSync('pkill -f "openclaw.*gateway"',{timeout:5000}); return {success:true,message:'Gateway killed'};}catch(e){return {success:false,message:e.message};} }
}
...
app.get('/api/dashboard', authMw, (req, res) => {
  ...
  try { execSync('openclaw gateway status 2>&1',{timeout:5000}); gwStatus='running'; } catch { try{execSync('pgrep -f "openclaw.*gateway"',{timeout:3000});gwStatus='running';}catch{gwStatus='stopped';} }
...
app.post('/api/gateway/restart', authMw, requireRole('admin'), async (req, res) => {
  try { execSync('openclaw gateway restart',{timeout:15000}); res.json({success:true,message:'Gateway restarted'}); }
  catch { try{execSync('pkill -f "openclaw.*gateway"',{timeout:5000});res.json({success:true,message:'Gateway restart initiated'});}catch(e){res.json({success:false,message:e.message});} }
});
app.get('/api/gateway/status', authMw, (req, res) => {
  let status='unknown',info='';
  try{info=execSync('openclaw gateway status 2>&1',{timeout:5000}).toString();status='running';}catch(e){info=e.message;status='stopped';}
...
// ─── Server + WebSocket ───
const server = app.listen(PORT, '0.0.0.0', () => console.log(`[clawdef] v3 — Token 优化平台 running on http://0.0.0.0:${PORT}`));
  ...
  ws.on('message', raw => {
    try {
      ...
      if (msg.type === 'subscribe_logs') {
        ...
        const tail = spawn('tail', ['-f', '-n', String(msg.lines||50), logFile]);
        ws._tail = tail;
        tail.stdout.on('data', d => { if(ws.readyState===WebSocket.OPEN) ws.send(JSON.stringify({type:'log_line',data:d.toString()})); });
        tail.on('close', () => { if(ws.readyState===WebSocket.OPEN) ws.send(JSON.stringify({type:'log_eof'})); });
      }
    } catch {}
  });
  ws.on('close', () => { clients.delete(ws); if(ws._tail) ws._tail.kill(); });
});
